...
const writingStyles = /** @type {[string, ...string[]]} */ (
  Object.values(deepl.WritingStyle)
);
const writingTones = /** @type {[string, ...string[]]} */ (
  Object.values(deepl.WritingTone)
);
...
class LanguagesList {
  ...
  static async create(direction) {
    if (direction != "source" && direction !== "target") {
      throw new Error(
        'LanguagesList needs to be called with "target" or "source"'
      );
    }
  ...
  normalize(code) {
    ...
    if (!this.list.some((lang) => lang.code === lowerCode)) {
      throw new Error(
        `Invalid language code: ${lowerCode}. Available codes: ${this.codesList}`
      );
    }
...
server.tool(
  ...
  "Translate a document file using DeepL API",
  {
    inputFile: z
      .string()
      .describe("Path to the input document file to translate"),
    outputFile: z
      .string()
      .optional()
      .describe(
...
async function rephraseText({ text, style, tone }) {
  try {
    ...
    const translation = /** @type {import('deepl-node').WriteResult} */ (
      result
    );
...
async function translateDocument({
  ...
  if (!outputFile) {
    ...
    outputFile = path.join(
      parsedPath.dir,
      `${parsedPath.name}_${langCodeSet1}${parsedPath.ext}`
    );
  }
  ...
  try {
    ...
    const result = await deeplClient.translateDocument(
      ...
      outputFile,
      sourceLangCode
        ? /** @type {import('deepl-node').SourceLanguageCode} */ (
            sourceLangCode
          )
        : null,
...
async function getGlossaryDictionaryEntries({
  ...
  try {
    ...
    throw new Error(
      `Failed to get glossary dictionary entries: ${error.message}`
    );
  }
}
...
function mcpContentifyText(param) {
  if (typeof param != "string" && !Array.isArray(param)) {
    throw new Error(
      "mcpContentifyText() expects a string or an array of strings"
    );
  }
